...
app.post('/start/', function (req, res) {
	...
	res.redirect('http://localhost:7001/0_consent.html');
	res.end();
});
...
app.post('/consentform/', function (req, res) {
	...
	if (req.session.usrid) {
		...
		res.redirect('http://localhost:7001/1_background.html');
		res.end();
	}
...
app.post('/worktask/', function (req, res) {
	...
	if (goingToWorkTask) {
		...
		if (randNum) {
			...
			res.redirect(`http://localhost:7001/5_podcast_t1.html`);
			res.end();
		}
		else {
			...
			res.redirect(`http://localhost:7001/8_presentation_t1.html`);
			res.end();
		}
	}
	else {
		...
		if (randNum) {
			...
			res.redirect(`http://localhost:7001/6_karaoke_t1.html`);
			res.end();
		}
		else {
			//Generate a random number, 1 to 8 inclusive, to determine which podcast task to go
			...
			res.redirect(`http://localhost:7001/7_audiobook_t1.html`);
			res.end();
		}
	}
...
// 		res.setHeader("Access-Control-Allow-Origin", "http://localhost:7001");
// 		res.redirect(`http://localhost:7001/6_karaoke_t${randTaskNum}.html`);
// 		res.send();
// 	} else {
// 		let randTaskNum = 1 + Math.floor(Math.random() * 5);
// 		res.setHeader("Access-Control-Allow-Origin", "http://localhost:7001");
// 		res.redirect(`http://localhost:7001/7_audiobook_t${randTaskNum}.html`);
// 		res.end();
// 	}
// });
